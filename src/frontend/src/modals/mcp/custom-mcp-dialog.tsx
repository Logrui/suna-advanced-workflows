...
// import { createClient } from '@/lib/supabase/client'; // TODO: Verify this path
...
export interface CustomMCPConfiguration {
    name: string;
    type: 'http';
    config: any;
    enabledTools: string[];
...
export const CustomMCPDialog: React.FC<CustomMCPDialogProps> = ({
    ...
    const validateAndDiscoverTools = async () => {
        ...
        try {
            ...
            // Replaced Supabase auth with standard backendApi call which handles auth internally
            ...
            if (!response.success) {
                throw new Error(response.error?.message || 'Failed to connect to the MCP server. Please check your configuration.');
            }
